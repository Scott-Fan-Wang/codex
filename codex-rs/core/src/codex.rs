...
async fn submission_loop(
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        match sub.op {
            ...
            Op::RequestExec { command, reason } => {
                // Execute a user-initiated shell command directly, using the same
                // approval/sandbox pipeline as model tool calls. Stream output and
                // emit ExecBegin/ExecEnd events.
                ...
                // Reuse the function-call handler to perform safety checks,
                // approvals, sandboxing and event emission. Discard the return
                // payload since this path does not reply to a model.
